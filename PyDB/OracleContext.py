...
class OracleContext(DbContext):
    ...
    def save_or_update(self, tablename, data):
        ...
        if key_signed:
            key_condition = 'and'.join([' %s = %s ' % (key.name, self.dialect.format_value_string(key, data[key.name])) if data.get(key.name, None) else ' %s is null ' % (key.name,) for key in key_fields])
    ...
    def exists_key(self, tablename, keys):
        ...
        if keys is not None:
            key_condition = 'and'.join([' %s = %s ' % (key.name, self.dialect.format_value_string(key, keys[key.name])) if keys.get(key.name, None) else ' %s is null ' % (key.name,) for key in key_fields])
            sql += ' where ' + key_condition
