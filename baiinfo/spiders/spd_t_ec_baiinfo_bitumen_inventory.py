...
from baiinfo.items import BitumenInventoryItem
import re
import datetime
...
class BitumenInventorySpider(scrapy.Spider):
    name = "spd_t_ec_baiinfo_bitumen_inventory"
    ...
    def parse(self, response):
        self.crawler.stats.set_value('spiderlog/source_name', u'百川资讯-库存')
        self.crawler.stats.set_value('spiderlog/target_tables', ['T_EC_BAIINFO_BITUMEN_INVENTORY'])
    ...
    def parse_content(self, response):
        ...
        for data in data_list[1:]:
            item = BitumenInventoryItem()
            item['datadate'] = datadate
            item['area'] = data[0]
            item['last_week_inventory'] = data[1]
            item['current_week_inventory'] = data[2]
            item['change'] = data[3]
            item['remark'] = data[4]
